...
export default function FileTree() {
  ...
  const { closeTab, closeTabsInFolder } = useTabStore()
  ...
  const confirmAddItem = () => {
    ...
    if (isAddingItem.type === "folder" && !expandedFolders.has(newPath)) handleToggleFolder(newPath)
      ...
      return (
        ...
        >
          <div
            ...
            data-component={node.type === "folder" ? "FileTreeFolderRow" : "FileTreeFileRow"}
          >
            {node.type === "folder" ? (
              <>
                <Button
                  ...
                  data-component="FileTreeToggle"
                >
                  {isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </Button>
                <Folder className="h-4 w-4 mr-2 text-blue-500" />
                <span className="flex-1" onClick={() => handleToggleFolder(node.path)} data-component="FileTreeFolderLabel">{node.path.split("/").pop()}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => handleAddItem("file", node.path)}
                  className="h-6 w-6"
                  aria-label="Add file"
                  data-component="FileTreeAddFile"
                ><Plus className="h-3 w-3" /></Button>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => handleRenameItem(node.path, node.path.split("/").pop() || "")}
                  className="h-6 w-6"
                  aria-label="Rename folder"
                  data-component="FileTreeRenameFolder"
  ...
  onClick={() => {
    ...
    if (node.type === "folder") {
      // 🔁 Delay to let Zustand update state first
      setTimeout(() => {
        closeTabsInFolder(node.path)
      }, 0)
    }
  }}
  className="h-6 w-6"
  aria-label="Delete folder"
  data-component="FileTreeDeleteFolder"
>
  <Trash className="h-3 w-3" />
              ...
              <>
                ...
                }} data-component="FileTreeFileLabel">{node.path.split("/").pop()}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => handleRenameItem(node.path, node.path.split("/").pop() || "")}
                  ...
                  data-component="FileTreeRenameFile"
                ><Edit className="h-3 w-3" /></Button>
                <Button
                  variant="ghost"
                  size="icon"
                  ...
                  data-component="FileTreeDeleteFile"
          ...
          {isRenaming && (
            <div className="flex items-center pl-4 py-1" data-component="FileTreeRenameItem">
              <Input
                ref={inputRef}
                value={newItemName}
                ...
                aria-label="New name"
              />
              <Button variant="ghost" size="icon" onClick={confirmRenameItem} className="h-7 w-7" aria-label="Save new name" data-component="FileTreeSaveRename"><Save className="h-4 w-4" /></Button>
              <Button variant="ghost" size="icon" onClick={cancelAction} className="h-7 w-7" aria-label="Cancel rename" data-component="FileTreeCancelRename"><X className="h-4 w-4" /></Button>
            </div>
          )}

          {node.type === "folder" && isExpanded && node.children && (
            <div className="ml-4" role="group" aria-label="Folder contents">
              {renderTree(node.children)}
              {isAdding && (
                <div className="flex items-center pl-4 py-1" data-component="FileTreeAddItem">
                  <Input
                    ref={inputRef}
                    value={newItemName}
                    onChange={(e) => setNewItemName(e.target.value)}
                  ...
                  />
                  <Button variant="ghost" size="icon" onClick={confirmAddItem} className="h-7 w-7" aria-label="Save new item" data-component="FileTreeSaveNewItem"><Save className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={cancelAction} className="h-7 w-7" aria-label="Cancel add item" data-component="FileTreeCancelAddItem"><X className="h-4 w-4" /></Button>
                </div>
              )}
            </div>
          )}
    ...
    <div className="h-full border-l overflow-y-auto p-2">
      <div className="flex items-center justify-between mb-4 px-2">
        ...
        <div className="flex space-x-1">
          <Button variant="outline" size="sm" onClick={() => handleAddItem("file", "")} className="h-7" aria-label="Add file" data-component="FileTreeAddFileButton">
            <File className="h-3.5 w-3.5 mr-1" /> Add File
          </Button>
          <Button variant="outline" size="sm" onClick={() => handleAddItem("folder", "")} className="h-7" aria-label="Add folder" data-component="FileTreeAddFolderButton">
            <Folder className="h-3.5 w-3.5 mr-1" /> Add Folder
          </Button>
        </div>
      </div>
      <nav className="file-tree" role="navigation" aria-label="File Tree" data-component="FileTreeNav">
