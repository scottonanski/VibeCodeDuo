...
import {
  ...
  type PipelineEvent,
  type PipelineParams, // Ensure this is imported
  type StageEvent      // Import if stages yield their own events that pipeline re-yields
} from './stages/types';
...
import { parseReviewOutput } from '@/lib/utils/jsonParser'; // Assuming you created this
...
function getTruncatedHistory(history: AiChatMessage[], refinedPrompt: string): AiChatMessage[] {
    ...
    if (history.length > 0) result.push(history[0]); // always keep first (system) message
...
export async function* collaborationPipeline(
  params: PipelineParams // Accept the whole params object
  ...
  const state: CollaborationState = {
      ...
      filename: params.filename, // Store current target filename in state
  };
  ...
  try {
      ...
      try {
          ...
          state.conversationHistory.push(...refineResult.messages); // Messages from refiner interaction
      } catch (error: any) {
      ...
      let lastWorker1FullTextResponse = ''; // To pass W1's full text to W2

      // --- CODING & REVIEW LOOP ---
      // TypeScript may warn about unreachable cases, but these are included for future safety and robustness.
      while (state.currentTurn < state.maxTurns && state.stage !== 'done' && state.stage !== 'error') {
          const currentFilename = state.filename || 'app/page.tsx'; // Ensure filename is available
          ...
          if (state.currentWorker === 'w1') {
              ...
              let tempAccumulatedW1Response = ""; // Accumulate chunks for W1's full text
                  ...
                  for await (const step of codegenStage({
                      ...
                      if (step.type === 'codegen-chunk') {
                          ...
                          yield { type: 'assistant_chunk', data: { worker: 'w1', chunk: step.data.content }};
                           ...
                           tempAccumulatedW1Response += step.data.content; // Assume code is also part of response
                          ...
                          const extractedCode = extractCodeFromMarkdown(lastWorker1FullTextResponse, 'tsx'); // Adjust lang if needed
                          if (extractedCode) {
